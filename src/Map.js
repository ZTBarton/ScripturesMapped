...
function Map({ markers }) {
    ...
    useEffect(() => {
        if (map) {
            ...
            const activeMarkers = newMarkerObjs.filter(marker => markers.map(m => m.id).includes(marker.id));
            panMap(activeMarkers);
        }
    }, [markers]);

    window.showLocation = (id, placename, latitude, longitude, viewLatitude, viewLongitude, viewTilt, viewRoll, viewAltitude, viewHeading) => {
