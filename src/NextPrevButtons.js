...
function NextPrevButtons({ currentLocation, navigateTo, sx }) {
    ...
    const getPreviousChapter = (currentLocation) => {
        if (currentLocation.chapter === 1) {
            if (currentLocation.volume.id === 1 && currentLocation.book.id === 101) {
                ...
                return {volume: newVolume, book: newBook, chapter: newBook.numChapters};
            } else {
                // if we are in the first chapter of a book
                const newBook = books[currentLocation.book.id - 1];
                return {volume: currentLocation.volume, book: newBook, chapter: newBook.numChapters};
            }
        } else {
  ...
  return (
    <ButtonGroup size="small" sx={{alignSelf: 'flex-end', color: theme.palette.primary.main, ...sx }}>
        <Button onClick={() => navigateTo(getPreviousChapter(currentLocation))} ><NavigateBeforeIcon /></Button>
        <Button onClick={() => navigateTo(getNextChapter(currentLocation))} ><NavigateNextIcon /></Button>
    </ButtonGroup>
  );
}
